...
class HotReloader {
  ...
  protected void reloadFile() {
    ...
    if (className == null) {
      throw new CompilationException(
          "Could not determine class name in file %s. File is empty or invalid ?".formatted(javaFile));
    }
  ...
  private static @Language("markdown") @NotNull String buildErrorMessage(Throwable error) {
    ...
    final String googleLink = "https://www.google.com/search?q=" + percentEncode(exceptionSimpleName + " " + errorMesssage);
    ...
    return """
        ...
        **Stacktrace**:
        ...
        """.formatted(exceptionSimpleName, errorMesssage, stackTrace, googleLink, chatGptLink);
  }
      ...
      if (success) {
        ...
        final String errorMessage = diagnosticsCollector.getDiagnostics().stream()
            .map(d -> String.format("[%s] %s:%d:%d %s",
                                    ...
                                    d.getLineNumber(),
                                    d.getColumnNumber(),
                                    d.getMessage(null))).collect(Collectors.joining("\n\n"));
  ...
  private @Nonnull byte[] loadClassBytes(final @Nonnull String className) {
    try {
      ...
      if (!Files.exists(classFile)) {
        LOG.error("Class file not found: {} for original class name {}", classFile, className);
