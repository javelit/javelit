...
public class CodeComponentE2ETest {
  ...
  void testCodeVariations(TestInfo testInfo) {
    JtRunnable app = () -> {
      ...
      Jt
          .code(
              "This is a very long line that should wrap when wrap lines is enabled and demonstrates the line wrapping functionality")
          .wrapLines(true)
          .use();
    ...
    PlaywrightUtils.runInBrowser(testInfo, app, page -> {
      ...
      // Test 2: Language null
      assertThat(page.locator("#app jt-internal-code").nth(1).locator("pre code.language-none")).isVisible(
          WAIT_1_SEC_MAX);
      ...
      String computedWidth = theCodeComponent
          .evaluate("el => getComputedStyle(el).getPropertyValue('--code-width')")
          .toString();
      String computedHeight = theCodeComponent
          .evaluate("el => getComputedStyle(el).getPropertyValue('--code-height')")
          .toString();
