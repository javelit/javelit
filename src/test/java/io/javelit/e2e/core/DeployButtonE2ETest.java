...
public class DeployButtonE2ETest {
  ...
  void testDeployButtonPresenceAndModal(TestInfo testInfo) {
    ...
    PlaywrightUtils.runInBrowser(testInfo, app, page -> {
      ...
      assertThat(page.getByRole(AriaRole.HEADING).filter(new Locator.FilterOptions().setHasText("Railway"))).isVisible(
          WAIT_1_SEC_MAX);
      assertThat(page
                     .getByRole(AriaRole.HEADING)
                     .filter(new Locator.FilterOptions().setHasText("Custom Deployment"))).isVisible(WAIT_1_SEC_MAX);
    });
  }
}
