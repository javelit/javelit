 ...
 * NOTE: the getClasspath used by the command here is the test jvm getClasspath - this is not the behaviour when calling from the jar but this is ok to ignore for this test
 */
public class RemoteFileE2ETest {
  ...
  void testRunRemoteFileViaPlainTextUrl() {
    Assumptions.assumeFalse(
        System.getenv("SKIP_GITHUB_REMOTE") != null,
        "SKIP_GITHUB_REMOTE env var is set - skipping GitHub remote file tests"
    );
    ...
    final Thread cliThread = new Thread(() -> {
      ...
      String[] args = {"run", remoteUrl, "--getPort", String.valueOf(port), "--no-browser"};
      Cli.main(args);
    });
  ...
  void testRunRemoteGitHubFolder() {
    ...
    final Thread cliThread = new Thread(() -> {
      // Use main method approach since subcommands are package-private
      String[] args = {"run", remoteFolderUrl, "--getPort", String.valueOf(port), "--no-browser"};
      Cli.main(args);
    });
    cliThread.setDaemon(true);
    cliThread.start();
  ...
  void testRunRemotePlainGitHubRepoUrl() {
    ...
    final Thread cliThread = new Thread(() -> {
      // Use main method approach since subcommands are package-private
      String[] args = {"run", repoUrl, "--getPort", String.valueOf(port), "--no-browser"};
      Cli.main(args);
    });
    cliThread.setDaemon(true);
    cliThread.start();
