...
public class RunnableReloader extends Reloader {
    ...
    public RunnableReloader(AppConfig config) {
        checkArgument(config.buildSystem == BuildSystem.RUNTIME,
                      "Class reloader only supports RUNTIME build systems.");
        checkArgument(config.appRunnable != null);
        this.runnable = config.appRunnable;
    }
